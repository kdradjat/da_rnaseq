...
def main():
    ...
    prop_list = list(np.arange(0.20, 1.05, 0.05))
    
    # logs DataFrame
    df_iter = pd.DataFrame(columns=['prop','iter','epoch','train_source_loss','valid_source_loss','test_loss','train_source_acc','valid_source_acc','test_acc'])
    df_prop = pd.DataFrame(columns=['prop','iter','epoch','train_source_loss','valid_source_loss','test_loss','train_source_acc','valid_source_acc','test_acc'])
    df_final = pd.DataFrame(columns=['prop','iter','epoch','train_source_loss','valid_source_loss','test_loss','train_source_acc','valid_source_acc','test_acc'])
    
    for prop in prop_list:
        df_prop = pd.DataFrame(columns=['prop','iter','epoch','train_source_loss','valid_source_loss','test_loss','train_source_acc','valid_source_acc','test_acc'])
        for i in range(3):
            # split with proportion 
            train_idx, val_idx = generate_indices(data_target, target_labels, prop=prop, val_prop=0.15, test_prop=0, rs=0)
            ...
            d_optimizer = Adam(model.discriminator.parameters(), lr=1e-4, weight_decay=1e-5)
            # losses
            ...
            # df epochs
            df_iter = pd.DataFrame(columns=['prop','iter','epoch','train_source_loss','valid_source_loss','test_loss','train_source_acc','valid_source_acc','test_acc'])
            df_iter['prop'] = [prop for k in range(len(train_source_acc_history))]
